...
bazel build tensorflow/tools/graph_transforms:summarize_graph && \
bazel-bin/tensorflow/tools/graph_transforms/summarize_graph \
--in_graph=train/frozen_inference_graph_xception_1358.pb
...
--transforms='
  ...
  bazel build tensorflow/tools/graph_transforms:transform_graph && \
bazel-bin/tensorflow/tools/graph_transforms/transform_graph \
--in_graph=train/bazel build tensorflow/tools/graph_transforms:summarize_graph && \
bazel-bin/tensorflow/tools/graph_transforms/summarize_graph \
--in_graph=tensorflow/examples/android/assets/frozen_inference_graph_xception_257_10K.pb \
--out_graph=tensorflow/examples/android/assets/optimized_xception_257_10K.pb --inputs='ImageTensor' --outputs='SemanticPredictions' \
--transforms='
